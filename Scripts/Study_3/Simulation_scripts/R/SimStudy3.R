...
for (i in 1:nsim)
{
  for (j in 1:length(samplesize))
  {
    for (n in 1:nrow(parasample_pop)) {
      ...
      brewfile <- paste( models$population_mode, ".brew", sep = "")
      run.template(brewfile,
                   envir = envir,
                   temp.filename.base =  models$population_model)
      lgsfile <- paste( models$population_model, ".lgs", sep = "")
      shell(
        paste(
          "C:\\Users\\liuy16\\Downloads\\LatentGOLD6.0\\lg60.exe",
          lgsfile,
          "/b"
        )
      )
      ...
      assign("condition", rownames(parasample_pop)[n], envir = envir) # label the entropy and proportion conditions
      assign("sampleparameters",
             unlist(parasample_onestepA[n, ]),
      ...
      lgsfile1 <- paste( models$onestep_a, ".lgs", sep = "")
      shell(
        paste(
          "C:\\Users\\liuy16\\Downloads\\LatentGOLD6.0\\lg60.exe",
          lgsfile1,
      ...
      assign("condition", rownames(parasample_onestepB)[n], envir = envir) 
      assign("sampleparameters",
             unlist(parasample_onestepB[n, ]),
             envir = envir)  
      brewfile2 <- paste(models$onestep_b, ".brew", sep = "")
      run.template(brewfile2,
                   envir = envir,
                   temp.filename.base = models$onestep_b) #create lgs files.
      lgsfile1 <- paste( models$onestep_b, ".lgs", sep = "")
      shell(
        paste(
          "C:\\Users\\liuy16\\Downloads\\LatentGOLD6.0\\lg60.exe",
          lgsfile1,
      ...
      assign("sampleparameters",
             unlist(parasample_onestepC[n, ]),
             envir = envir)  
      brewfile3 <- paste( models$onestep_c, ".brew", sep = "")
      run.template(brewfile3,
                   envir = envir,
                   temp.filename.base = models$onestep_c) #create lgs files.
      lgsfile1 <- paste( models$onestep_c, ".lgs", sep = "")
      shell(
        paste(
          "C:\\Users\\liuy16\\Downloads\\LatentGOLD6.0\\lg60.exe",
          lgsfile1,
      ...
      assign("size", rownames(samplesize)[j], envir = envir) 
      assign("sampleparameters",
             unlist(parasample_steponeA[n, ]),
             envir = envir)  # 2) fix the starting values
      assign("condition", rownames(parasample_steponeA)[n], envir = envir) 
      brewfile4 <- paste( models$steponeA, ".brew", sep = "")
      run.template(brewfile4,
                   envir = envir,
                   temp.filename.base = models$steponeA)
      lgsfile2 <- paste( models$steponeA, ".lgs", sep = "")
      shell(
        paste(
          "C:\\Users\\liuy16\\Downloads\\LatentGOLD6.0\\lg60.exe",
          lgsfile2,
      ...
      brewfile5 <- paste( models$steptwoA, ".brew", sep = "")
      run.template(brewfile5,
                   envir = envir,
                   temp.filename.base = models$steptwoA) 
      lgsfile4 <- paste( models$steptwoA , ".lgs", sep = "")
      shell(
        paste(
          "C:\\Users\\liuy16\\Downloads\\LatentGOLD6.0\\lg60.exe",
          lgsfile4,
      ...
      assign("size", rownames(samplesize)[j], envir = envir) 
      assign("sampleparameters",
             unlist(parasample_steponeB[n, ]),
             envir = envir)  
      assign("condition", rownames(parasample_onestepB)[n], envir = envir) 
      brewfile6 <- paste( models$steponeB, ".brew", sep = "")
      run.template(brewfile6,
                   envir = envir,
                   temp.filename.base = models$steponeB)
      lgsfile2 <- paste( models$steponeB, ".lgs", sep = "")
      shell(
        paste(
          "C:\\Users\\liuy16\\Downloads\\LatentGOLD6.0\\lg60.exe",
          lgsfile2,
      ...
      bpars <- as.matrix(read.table(DAT, sep = " ", dec = ","))
      envir <- new.env()
      assign("condition", rownames(parasample_steponeB)[n], envir = envir) 
      assign("size", rownames(samplesize)[j], envir = envir) 
      assign("step1b_pars", bpars[-c(1, 2), ], envir = envir)
      
      brewfile7 <- paste( models$steptwoB, ".brew", sep = "")
      run.template(brewfile7,
                   envir = envir,
                   temp.filename.base = models$steptwoB) 
      lgsfile4 <- paste( models$steptwoB , ".lgs", sep = "")
      shell(
        paste(
          "C:\\Users\\liuy16\\Downloads\\LatentGOLD6.0\\lg60.exe",
          lgsfile4,
      ...
      assign("size", rownames(samplesize)[j], envir = envir)
      assign("sampleparameters",
             unlist(parasample_steponeC[n, ]),
             envir = envir)  
      assign("condition", rownames(parasample_steponeC)[n], envir = envir) 
      brewfile8 <- paste( models$steponeC, ".brew", sep = "")
      run.template(brewfile8,
                   envir = envir,
                   temp.filename.base = models$steponeC)
      lgsfile2 <- paste( models$steponeC, ".lgs", sep = "")
      shell(
        paste(
          "C:\\Users\\liuy16\\Downloads\\LatentGOLD6.0\\lg60.exe",
          lgsfile2,
      ...
      ## step 2 B model
      brewfile9 <- paste( models$steptwoC, ".brew", sep = "")
      run.template(brewfile9,
                   envir = envir,
                   temp.filename.base = models$steptwoC)
      lgsfile4 <- paste( models$steptwoC , ".lgs", sep = "")
      shell(
        paste(
          "C:\\Users\\liuy16\\Downloads\\LatentGOLD6.0\\lg60.exe",
          lgsfile4,
      ...
      assign("condition",
             rownames(parasample_steponeA)[n],
             envir = envir)
      assign("size", rownames(samplesize)[j], envir = envir)
      brewfile <- paste( models$stepthreeA, ".brew", sep = "")
      run.template(brewfile,
                   envir = envir,
                   temp.filename.base = models$stepthreeA) 
      lgsfile4 <- paste( models$stepthreeA, ".lgs", sep = "")
      shell(
        paste(
          "C:\\Users\\liuy16\\Downloads\\LatentGOLD6.0\\lg60.exe",
          lgsfile4,
      ...
      for (s1_model in modeltype[-1]) {
        ...
        assign("condition",
               ...
               envir = envir)
        assign("size", rownames(samplesize)[j], envir = envir)
        brewfile <- paste( models$stepthree, ".brew", sep = "")
        run.template(brewfile,
                     envir = envir,
                     temp.filename.base = models$stepthree) 
        lgsfile4 <- paste( models$stepthree, ".lgs", sep = "")
        shell(
          paste(
            "C:\\Users\\liuy16\\Downloads\\LatentGOLD6.0\\lg60.exe",
            lgsfile4,
