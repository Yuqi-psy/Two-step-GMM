...
parasample_pop <- expand.grid(Int_lc=Int_lc, Slo_lc=Slo_lc_cov12, # regression coefficient for the latent class variable
                              ...
                              Slo_s0=Slo_s0, Slo_S1=Slo_s1,# regression coefficient for the latent slope variable
                               Var_items=Var_items3,
...
setwd("C:\\Users\\liuy16\\Desktop\\PhD files\\Project1\\Simulated_data\\Study_3\\3T")


# specify models
population_model <- c("DatGen_3T") # population model

onestep_a <- c("onestepA_3T")
onestep_b <- c("onestepB_3T") 
onestep_c <- c("onestepC_3T")# one step model

steponeA <- c("step1A_3T")
steponeB <- c("step1B_3T") 
steponeC <- c("step1C_3T") # step one model

steptwoA1 <- c("step2A1_3T")
steptwoA2 <- c("step2A2_3T") 
steptwoB <- c("step2B_3T") # step two model

stepthree <- c("step3_3T") # step three model
...
st-et
#### 1. in study equal condition, the overall mean keep in the same, it is matter or not? should we change the settings?
